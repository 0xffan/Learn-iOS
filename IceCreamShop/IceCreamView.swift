...
class IceCreamView: UIView {
	...
	@IBInspectable var iceCreamTopColor = Flavor.vanilla().topColor {
		didSet {
			setNeedsDisplay()
		}
	}
	@IBInspectable var iceCreamBottomColor = Flavor.vanilla().bottomColor {
		didSet {
			setNeedsDisplay()
		}
	}
    ...
    override func drawRect(frame: CGRect) {
		...
		CGContextDrawLinearGradient(context, iceCreamGradient, CGPointMake(scoopBounds.midX, scoopBounds.minY), CGPointMake(scoopBounds.midX, scoopBounds.maxY), CGGradientDrawingOptions(rawValue: 0))
		CGContextEndTransparencyLayer(context)
		CGContextRestoreGState(context)
    }
}
